...
def detect_entities(
    ...
    Args:
        ...
        corpus_origin: Optional Phase 1 corpus_origin context. When supplied
            and the corpus is identified as AI-dialogue with known agent
            persona names, candidates whose name matches an agent persona
            are moved out of ``people``/``uncertain`` and into a new
            ``agent_personas`` bucket. Shape:
...
def _apply_corpus_origin(detected: dict, corpus_origin: dict | None) -> dict:
    """Reclassify per-candidate buckets using Phase 1 corpus_origin context.
