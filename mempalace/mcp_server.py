...
def tool_add_drawer(
    ...
    effective_source = (
        source_file or f"mcp:{hashlib.md5(content.encode(), usedforsecurity=False).hexdigest()}"
    )
    drawer_id = (
        f"drawer_{wing}_{room}_{hashlib.sha256((effective_source + '0').encode()).hexdigest()[:24]}"
    )
