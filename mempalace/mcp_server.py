...
# ── Fix: Windows UTF-8 mode for stdin/stdout ────────────────────────────────
if sys.platform == "win32":
    # Force UTF-8 mode at the buffer level — must happen BEFORE any I/O
    import builtins
    _original_open = builtins.open
    
    # Override stdin/stdout at the file descriptor level for binary-correct reads
    import io
    sys.stdin = io.TextIOWrapper(
        io.FileIO(sys.stdin.fileno(), mode="r"),
        encoding="utf-8",
        errors="surrogateescape",
        line_buffering=True,
    )
    sys.stdout = io.TextIOWrapper(
        io.FileIO(sys.stdout.fileno(), mode="w"),
        encoding="utf-8",
        errors="surrogateescape",
        line_buffering=True,
    )
    sys.stderr = io.TextIOWrapper(
        io.FileIO(sys.stderr.fileno(), mode="w"),
        encoding="utf-8",
        errors="replace",
        line_buffering=True,
    )

# ── Fix: Strip Python surrogate characters that cause encoding crashes ────────
_SENTINEL_REPLACEMENT = "\ufffd"


def _strip_surrogates(text: str) -> str:
    """Remove Python surrogate characters that crash on Windows UTF-8 encode."""
    if not isinstance(text, str):
        return text
    # Use surrogateescape to preserve valid chars while removing lone surrogates
    return text.encode("utf-8", errors="surrogatepass").decode("utf-8", errors="replace")


def _sanitize_params(params: dict) -> dict:
    """Recursively sanitize all string values in params dict."""
    if params is None:
        return params
    sanitized = {}
    for k, v in params.items():
        if isinstance(v, str):
            sanitized[k] = _strip_surrogates(v)
        elif isinstance(v, dict):
            sanitized[k] = _sanitize_params(v)
        elif isinstance(v, list):
            sanitized[k] = [
                _strip_surrogates(i) if isinstance(i, str) else i for i in v
            ]
        else:
            sanitized[k] = v
    return sanitized
...
def handle_request(request):
    ...
    # ── Fix: sanitize params to strip surrogates before any processing ────
