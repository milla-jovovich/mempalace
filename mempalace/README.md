...
| `knowledge_graph.py` | Temporal entity-relationship graph — SQLite with Row factory, time-filtered queries, fact invalidation |
| `palace_graph.py` | Room-based navigation graph — BFS traversal, tunnel detection across wings |
| `palace_db.py` | Singleton ChromaDB client, cached collections, shared query helpers |
| `mcp_server.py` | MCP server — 14 tools, AAAK auto-teach, Palace Protocol, agent diary |
