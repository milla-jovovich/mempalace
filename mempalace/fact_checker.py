...
def _check_kg_contradictions(text: str, palace_path: str) -> list:
    ...
    issues: list = []
    # The KG connection is closed on the way out (including on an early
    # raise): this helper runs per fact-check, so leaking a SQLite handle
    # per call is what starves peer writers of the write lock.
    with kg:
        for claim in claims:
