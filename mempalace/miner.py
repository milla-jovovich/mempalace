...
def detect_room(filepath: Path, content: str, rooms: list, project_path: Path) -> str:
    ...
    # Priority 1: explicit 'path' in room config (exact match)
    for room in rooms:
        room_path = room.get("path", "").lower()
        if room_path and relative.startswith(room_path):
            return room["name"]
    
    # Priority 1b: folder path contains room name (fallback)
    path_parts = relative.replace("\\", "/").split("/")
    for part in path_parts[:-1]:  # skip filename itself
        for room in rooms:
            room_name = room["name"].lower()
            # Handle underscores vs dashes: core_java <-> core-java
            normalized = room_name.replace("_", "-")
            if (room_name in part or part in room_name or
                normalized in part or part in normalized):
                return room["name"]
...
def file_already_mined(collection, source_file: str, content_hash: str = None) -> tuple[bool, list]:
    """
    Check if file was already mined AND whether content changed.
    Returns: (is_unchanged, existing_drawer_ids)
    - is_unchanged=True → content identical, skip file
    - is_unchanged=False → content changed or new file, re-index needed
    """
    try:
        results = collection.get(where={"source_file": source_file}, include=["metadatas"])
        drawer_ids = results.get("ids", [])
        if not drawer_ids:
            return False, []  # New file

        # Check stored hash in first drawer's metadata
        metadatas = results.get("metadatas", [])
        if metadatas and content_hash:
            stored_hash = metadatas[0].get("content_hash")
            if stored_hash == content_hash:
                return True, drawer_ids  # Content unchanged
        return False, drawer_ids  # Content changed or no hash
    except Exception:
        return False, []
...
def add_drawer(
    ...
    chunk_index: int, agent: str, content_hash: str = None
):
...
def process_file(
    ...
    if not dry_run:
        is_unchanged, old_drawer_ids = file_already_mined(collection, source_file, content_hash)
        if is_unchanged:
            return 0  # Content unchanged, skip
        elif old_drawer_ids:
            # Content changed — delete old drawers first
            for old_id in old_drawer_ids:
                try:
