...
_AI_UNAMBIGUOUS_TERMS = [
    # Anthropic-specific
    "Anthropic", "Claude Code", "Claude 3", "Claude 4", "claude mcp",
    "CLAUDE.md", ".claude/",
    # OpenAI-specific
    "ChatGPT", "GPT-4", "GPT-3", "GPT-5", "OpenAI", "gpt-4o", "gpt-4-turbo",
    "o1-preview", "o3",
    # Google-specific
    "gemini-pro", "gemini-1.5", "Google AI",
    # Meta / others (specific model identifiers, not bare common words)
    "Mixtral", "Cohere",
    # AI-infrastructure terms with no common-English collision
    "MCP", "LLM", "RAG", "fine-tune", "context window", "embedding",
]
...
_AI_AMBIGUOUS_TERMS = [
    ...
    # removed (Igor's review §6.4): they're suppressed without an unambiguous
    # co-signal anyway, and by the time a co-signal is present the corpus is
    # already flagged. Keeping them just produced noisier evidence strings.
]
...
def detect_origin_heuristic(samples: list[str]) -> CorpusOriginResult:
    ...
    if shown_hits:
        ...
        evidence.append(
            "AI brand terms: " + ", ".join(f"'{k}' ({v}x)" for k, v in top_terms)
        )
    ...
    if turn_hits:
        evidence.append(f"Turn markers detected: {turn_hits} occurrences across {len(turn_types_found)} pattern types")
