...
{
  "decision": "block",
  "reason": "COMPACTION IMMINENT — Save EVERYTHING before context is compressed. CRITICAL: Save tool output VERBATIM — Bash command results, probe findings, USB descriptors, firmware bytes, search results, build output, error messages. These are lost on compaction and exist nowhere else. Also save all topics, decisions, quotes, code, and important context. Be thorough — after compaction, detailed context will be lost. Organize into appropriate categories. Save everything, then allow compaction to proceed."
}
HOOKJSON
