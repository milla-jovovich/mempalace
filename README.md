...
|---|---|---|---|
| **Reliability** | Skip `_fix_blob_seq_ids` sqlite open after first successful migration via `.blob_seq_ids_migrated` marker — opening sqlite3 against a live ChromaDB 1.5.x file corrupts the next PersistentClient | PR candidate (issue-first: narrow chromadb 1.5.x debugging path, needs context before a PR lands) | `backends/chroma.py` |
| **Reliability** | `_get_client()` tries `get_collection` before `create_collection` — `get_or_create_collection` segfaults ChromaDB 1.5.x when the existing collection's metadata differs from the call-site metadata | PR candidate (issue-first: chromadb bug; upstream may prefer a chroma-core report instead of a workaround) | `backends/chroma.py` |
...
| **Performance** | `bulk_check_mined()` paginated pre-fetch + `--workers` ThreadPoolExecutor concurrent mining | PR candidate (issue-first: overlaps in intent with upstream's file-locking in [#784](https://github.com/milla-jovovich/mempalace/pull/784), worth aligning before a PR) | `palace.py`, `miner.py` |
