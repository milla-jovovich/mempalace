...
- **Silent-save hook architecture** — the stop hook writes to the palace directly via the Python API, advances the save marker only after a confirmed write, and surfaces a `systemMessage` line in the terminal. Deterministic, zero data loss, no dependency on the AI acting on a `block` prompt. ([#673](https://github.com/milla-jovovich/mempalace/pull/673), APPROVED externally.) **Note — Claude Code 2.1.114 regression:** the terminal `<Line>` for `systemMessage` no longer persists on screen (it flashes for ~1s and disappears). Saves still complete correctly and the data lands in the palace; only the visible `✦ N memories woven into the palace` notification is gone. Tracked upstream at [anthropics/claude-code#50542](https://github.com/anthropics/claude-code/issues/50542). Producer-side fixes for fd-level stdout delivery and `stop_hook_active` handling in [#1021](https://github.com/milla-jovovich/mempalace/pull/1021); enable `hook_desktop_toast: true` in `~/.mempalace/config.json` for a `notify-send` popup as a visible substitute.
