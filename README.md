...
| **Performance** | `bulk_check_mined()` paginated pre-fetch + `--workers` ThreadPoolExecutor concurrent mining | PR candidate (issue-first — overlaps in intent with upstream's merged file-locking [#784](https://github.com/MemPalace/mempalace/pull/784); discussion needs to establish whether parallelism on top of locking is the right next step before a PR) | medium | medium | `palace.py`, `miner.py` |
