...
15. ~~**fix: `_get_client()` get-then-create guard**~~ — **NOT currently in fork main; #1262 (Legion345) in flight to land it upstream**. Guard landed in fork as `d3a2d22` on 2026-04-18 (`ChromaBackend.get_collection` + `mcp_server._get_collection`), then was silently dropped during the develop merge-conflict resolution in `7e18a70` on 2026-04-25. Current fork main has **four `get_or_create_collection` call sites with non-uniform metadata**: `backends/chroma.py:1040` and `mcp_server.py:295` pass the full `hnsw:*` set (`hnsw:space`, `hnsw:num_threads: 1`, `_HNSW_BLOAT_GUARD`); `migrate.py:235` and `cli.py:905` pass no metadata at all. The trigger condition for the ChromaDB 1.5.x metadata-mismatch SIGSEGV (call-site metadata diverging from stored collection metadata) is structurally present — the fork's apparent stability is not because metadata is consistent (it isn't) but presumably because the no-metadata sites operate on different collections (`mempalace_compressed`, the migrate temp collection) than the full-metadata sites (`mempalace_drawers`). Issue #1089 was filed 2026-04-21 documenting the pattern; #1262 (filed 2026-04-28 by @Legion345) implements it as `try get_collection / except chromadb.errors.NotFoundError → create_collection` — catching the renamed-in-1.5.x exception, an improvement over our older `InvalidCollectionException` reference. Once #1262 merges, our row clears via develop sync; the four call sites should be audited together at that point so the guard wraps every site, not just `chroma.py`. **Earlier 2026-04-30 wording said "stable metadata" / "consistency across opens" — that was wrong; corrected here.**
