...
21. **feat: `kind` filter on `search_memories` excludes Stop-hook checkpoints by default** (commit `8d02835`, 2026-04-25) — Stop-hook auto-save diary entries (topic=checkpoint, text starting `"CHECKPOINT:"`) were dominating MCP search results because they're short, word-dense, and outrank substantive content under cosine similarity. Validated end-to-end against the canonical daemon palace on 2026-04-25 — `kind="all"` (legacy default) returned 5 CHECKPOINT entries for "palace-daemon migration architecture"; `kind="content"` (new default) returned substantive content drawers from realm_sigil and configuration rooms. New `kind` parameter on `search_memories` and `mempalace_search` MCP tool: `"content"` (default, excludes checkpoints), `"checkpoint"` (only checkpoints, recovery/audit), `"all"` (no filter, pre-2026-04-25 behavior). Defense-in-depth: where-clause filter on `topic` metadata + post-filter on `text.startswith("CHECKPOINT:")` so legacy data with `metadata=None` or missing topic field is also caught. Result dicts now surface `topic`. 8 new tests in `TestCheckpointFilter`. Companion fix in [`jphein/palace-daemon`](https://github.com/jphein/palace-daemon) commit `dd8894c` standardizes all hook clients on `topic="checkpoint"` (was `topic="auto-save"` in `clients/hook.py`). Upstream PR pending.
