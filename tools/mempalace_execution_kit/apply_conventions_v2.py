...
def derive(cfg: dict) -> dict:
    ...
    target_id = tgt['id']
    display = ov.get('plugin_display_name') or tgt.get('display_name') or target_id
    package = target_id
    ...
    return {
        ...
        'plugin_display_name': display,
        'brand_color': tgt.get('brand_color', '#2563EB'),
        'interpreter_policy': {
            'env_var': interpreter_env_var,
            'fallback_command': cfg.get('interpreter_policy', {}).get('fallback_command', 'python')
        }
    }
...
def main():
    if len(sys.argv) != 3:
        print('Usage: python apply_conventions_v2.py /path/to/repo /path/to/conventions.json')
        raise SystemExit(2)
    ...
    print('Derived target configuration:')
