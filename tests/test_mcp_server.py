...
class TestHandleRequest:
    ...
    def test_extra_kwargs_filtered(self, monkeypatch, config, palace_path, seeded_kg):
        """Extra arguments not in the tool's input_schema must be silently dropped."""
        _patch_mcp_server(monkeypatch, config, palace_path, seeded_kg)
        _get_collection(palace_path, create=True)
        from mempalace.mcp_server import handle_request
        ...
        resp = handle_request({
            ...
            "params": {
                ...
                "arguments": {"injected_kwarg": "malicious_value", "palace_path": "/etc/shadow"},
            },
        })
        # Should succeed — extra kwargs are dropped, not passed to the handler
        assert "result" in resp
...
class TestInputValidation:
    def test_oversized_content_rejected(self, monkeypatch, config, palace_path, kg):
    ...
    def test_oversized_query_rejected(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, palace_path, kg)
        from mempalace.mcp_server import handle_request
        ...
        resp = handle_request({
            ...
            "params": {
                ...
                "arguments": {"query": "x" * 3_000},
            },
        })
        assert "error" in resp

    def test_limit_clamped_to_max(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, palace_path, kg)
        from mempalace.mcp_server import tool_search

        # limit=999 should be clamped to MAX_RESULTS_LIMIT (100)
        result = tool_search(query="test", limit=999)
        # No error — it was silently clamped
