...
class TestHeuristic:
    def test_claude_heavy_corpus_detected(self):
        ...
        assert "Claude" in " ".join(result.evidence) or "claude" in " ".join(result.evidence).lower()
    ...
    def test_word_boundary_brand_matching(self):
        ...
        Per Igor's review §6.3: this doesn't change classification on the
        co-occurrence-suppressed cases, but it cleans up the evidence
        strings — false matches must not appear in the audit trail.
        It also prevents 'Claude Code' from triple-counting as
        'Claude Code' + 'Claude' overlap."""
        samples = [
...
class TestLLMConfirmation:
    ...
    def test_handles_malformed_llm_response(self):
        ...
        assert "fallback" in " ".join(result.evidence).lower() or "error" in " ".join(result.evidence).lower()
